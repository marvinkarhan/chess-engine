...
def on_make_move(uci_move: str):
    ...
    best_moves_strings = [move.to_uci_string()
                          for move in reversed(best_moves)]
    emit('new_board_info', {'fen': user_board.to_fen_string(
    ), 'moves': uci_moves, 'evaluation': score / 100, 'aiMoves': best_moves_strings})
...
def on_new_board():
    ...
    uci_moves = [move.to_uci_string() for move in moves]
    emit('new_board_info', {'fen': user_board.to_fen_string(
    ), 'moves': uci_moves, 'evaluation': user_board.evaluate() / 100})
