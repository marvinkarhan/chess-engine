...
namespace NNUE
{
  ...
  template <typename PreviousLayer, std::uint32_t OutputDimensions>
  ...
  public:
    ...
    static constexpr std::uint32_t kPaddedInputDimensions =
        CeilToMultiple<std::uint32_t>(kInputDimensions, kMaxSimdWidth);
        ...
        const std::uint8_t *transformed_features, char *buffer) const
    {
      ...
      constexpr std::uint32_t kNumChunks = kPaddedInputDimensions / (kSimdWidth * 2);
      ...
      {
        const std::uint32_t offset = i * kPaddedInputDimensions;
        ...
        for (std::uint32_t j = 0; j < kInputDimensions; ++j)
        {
